...
export default (state = initialState, action) => {
  ...
  case 'CHON_CHO': {
    let {ghe} = action;
    let exist = state.danhSachGheChon.find(item => item.soGhe === ghe.soGhe);
    if(!exist) {
      state.danhSachGheChon = [...state.danhSachGheChon, ghe];
    } else {
      state.danhSachGheChon = [...state.danhSachGheChon.filter(item => item.soGhe != ghe.soGhe)];
    }
    return {...state}
  }
  case 'BO_CHON': {
    state.danhSachGheChon = [...state.danhSachGheChon.filter(item => item.soGhe != action.soGhe)];
    return {...state};
  }
  default:
    return state;
